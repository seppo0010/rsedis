start_server {tags {"slowlog"} overrides {slowlog-log-slower-than 1000000}} {
    xtest {SLOWLOG - check that it starts with an empty log} {
        r slowlog len
    } {0}

    xtest {SLOWLOG - only logs commands taking more time than specified} {
    ...
    xtest {SLOWLOG - max entries is correctly handled} {
    ...
    xtest {SLOWLOG - GET optional argument to limit output len works} {
        llength [r slowlog get 5]
    } {5}

    xtest {SLOWLOG - RESET subcommand works} {
        r config set slowlog-log-slower-than 100000
        r slowlog reset
        r slowlog len
    } {0}

    xtest {SLOWLOG - logged entry sanity check} {
        r debug sleep 0.2
    ...
    xtest {SLOWLOG - commands with too many arguments are trimmed} {
        r config set slowlog-log-slower-than 0
        r slowlog reset
    ...
    xtest {SLOWLOG - too long arguments are trimmed} {
        r config set slowlog-log-slower-than 0
        r slowlog reset
    ...
    xtest {SLOWLOG - EXEC is not logged, just executed commands} {
        r config set slowlog-log-slower-than 100000
        r slowlog reset
        assert_equal [r slowlog len] 0
        r multi
