start_server {tags {"introspection"}} {
    ...
    xtest {MONITOR can log executed commands} {
    ...
    xtest {CLIENT GETNAME should return NIL if name is not assigned} {
        r client getname
    } {}

    xtest {CLIENT LIST shows empty fields for unassigned names} {
        r client list
    } {*name= *}

    xtest {CLIENT SETNAME does not accept spaces} {
        catch {r client setname "foo bar"} e
        set e
    } {ERR*}

    xtest {CLIENT SETNAME can assign a name to this connection} {
        assert_equal [r client setname myname] {OK}
        r client list
    } {*name=myname*}

    xtest {CLIENT SETNAME can change the name of an existing connection} {
        assert_equal [r client setname someothername] {OK}
        r client list
    } {*name=someothername*}

    xtest {After CLIENT SETNAME, connection can still be closed} {
        set rd [redis_deferring_client]
